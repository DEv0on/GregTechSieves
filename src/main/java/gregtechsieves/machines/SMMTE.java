...
public class SMMTE extends WorkableTieredMetaTileEntity {
    ...
    public SMMTE(ResourceLocation metaTileEntityId, RecipeMap<?> recipeMap, OrientedOverlayRenderer renderer, int tier) {
        this(metaTileEntityId, recipeMap, renderer, tier, true);
    }

    public SMMTE(ResourceLocation metaTileEntityId, RecipeMap<?> recipeMap, OrientedOverlayRenderer renderer, int tier, boolean hasFrontFacing) {
    ...
    public MetaTileEntity createMetaTileEntity(MetaTileEntityHolder holder) {
        return new SMMTE(metaTileEntityId, workable.recipeMap, renderer, getTier(), hasFrontFacing);
    }
    ...
    protected ModularUI.Builder createGuiTemplate(EntityPlayer player) {
        ModularUI.Builder builder = workable.recipeMap.createUITemplate(workable::getProgressPercent, importItems, exportItems, importFluids, exportFluids)
                ...
                .widget(new DischargerSlotWidget(chargerInventory, 0, 100, 26)
                        .setBackgroundTexture(GuiTextures.SLOT, GuiTextures.CHARGER_OVERLAY))
                .widget(new ImageWidget(80, 25, 18, 18, GuiTextures.INDICATOR_NO_ENERGY)
                        .setPredicate(workable::isHasNotEnoughEnergy))
                .widget(new LabelWidget(8,139, "Inventory:"))
                .bindPlayerInventory(player.inventory, GuiTextures.SLOT, 7, 150);



        if (exportItems.getSlots() > 0) {
            builder.widget(new ToggleButtonWidget(120, 25, 18, 18,
                    GuiTextures.BUTTON_ITEM_OUTPUT, this::isAutoOutputItems, this::setAutoOutputItems)
                    .setTooltipText("gregtech.gui.item_auto_output.tooltip"));
        }

        builder.widget(new ToggleButtonWidget(145, 24, 20, 20,
